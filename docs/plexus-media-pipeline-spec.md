...
- Fallback path: WebGPU now ships by default across Chrome, Firefox, and Safari (as of 2026), putting it in the ~80%+ global support range — solid enough to build on directly. Still worth a `navigator.gpu` feature-detection check with a reduced-fidelity Canvas2D/WebGL fallback for the remaining older browsers/devices, rather than hard-blocking the editor on WebGPU support.
...
- Which composite sliders (Light, Color, B&W, Sharpen, etc.) and what underlying parameters each one maps to — this needs its own small design pass before implementation, since it's the crux of "curated vs. raw."
