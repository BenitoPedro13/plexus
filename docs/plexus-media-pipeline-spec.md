...
**Edit Recipe** — the concept that unifies the editor with the pipeline engine. Every adjustment a user makes in the editor (crop, light, color, filter, format) is stored as a parameter, never a pixel mutation — the same non-destructive model Apple Photos uses. A recipe is just a small, ordered list of `{ processor, params }` entries — which is *structurally identical* to a Plexus pipeline. "Edit one image" and "define a pipeline" become the same underlying data structure, viewed through two different UIs (direct-manipulation editor vs. YAML/preset). This is what makes "edit once, apply to 500 files" nearly free architecturally instead of a separate feature. Concrete type: `packages/recipe/src/schema.ts` (`docs/tasks/TASK-recipe-schema.md`, extracted to a shared workspace package by `docs/tasks/TASK-recipe-packages-extraction.md`) — a Zod schema covering all eight P0 image processors (`image.resize`, `image.convert`, `image.compress`, `image.crop`, `image.adjustLight`, `image.adjustColor`, `image.blackAndWhite`, `image.sharpen`). Both `apps/web` (via a thin re-export shim at its old `src/lib/recipe/schema.ts` path, kept so its ~17 existing importers don't need repointing) and `apps/orchestrator` (`src/pipelines/dto/create-pipeline.dto.ts`, which now sources its accepted processor ids and per-step param validation from this same package instead of a hand-maintained, previously-stale copy) import `@plexus/recipe` — the "same data structure, two UIs" claim is now literally true in code, not just structurally similar (see `docs/90-deferred-register.md`, resolved `D-1`/`D-17`).
...
- Which composite sliders (Light, Color, B&W, Sharpen, etc.) and what underlying parameters each one maps to — **partially resolved 2026-08-07, see `docs/tasks/TASK-composite-slider-mapping.md` and `docs/90-deferred-register.md` D-19.** Four processor ids decided (`image.adjustLight`, `image.adjustColor`, `image.blackAndWhite`, `image.sharpen`) with a P0 param subset mapped to concrete govips calls. Highlights/Shadows resolved 2026-08-07 (`docs/tasks/TASK-highlights-shadows-tonelut.md`, `V-7`) — implemented Go-side via a local govips fork (`D-24`); live-preview/editor-UI parity for the two params also resolved 2026-08-07 (`docs/tasks/TASK-highlights-shadows-preview-parity.md`, `D-25`) — Lab-space `tonelut`-equivalent curve in both WGSL/GLSL, manual sliders in `LightControl.tsx`. Vibrance/Cast/Grain primitives researched 2026-08-07 (`docs/tasks/TASK-vibrance-cast-grain-spike.md`, resolved `V-8`): Cast implemented Go-side 2026-08-07 (`docs/tasks/TASK-adjust-color-cast.md`, resolved `D-27`) — grey-world white-balance via `ExtractBandToImage`/`Average`/`Linear`; live-preview/editor-UI parity also resolved 2026-08-07 (`docs/tasks/TASK-color-cast-preview-parity.md`, `D-30`) — a GPU box-downsample-pyramid mean reduction (both renderers) feeding the same grey-world scale, direct `Cast` slider in `ColorControl.tsx`. Grain implemented Go-side 2026-08-07 (`docs/tasks/TASK-black-and-white-grain.md`, resolved `D-28`) — a small govips-fork extension mirroring `D-24`'s `Tonelut` precedent (`workers/internal/govips-fork/vips/gaussnoise.go`, `NewGaussnoiseImage`), zero-mean noise added via plain arithmetic `Add` (not `Composite`/`BlendMode` — verified to synthesize an unwanted alpha band) with a documented first-pass sigma cap; live-preview/editor-UI parity also resolved 2026-08-07 (`docs/tasks/TASK-grain-preview-parity.md`, `D-31`) — a `sin`-hash → Box-Muller pseudo-random noise pass in both WGSL/GLSL (deliberately not a reproduction of libvips' own PRNG, only its statistical shape; validated with a statistical test rather than `drift.test.ts`'s per-pixel methodology, `D-32`), direct `Grain` slider in `BlackAndWhiteControl.tsx`. Vibrance's exact curve has no primary-source-correct answer and stays a visual judgment call (`D-29`, now also covering Grain's sigma-cap/seed choices, folded into `D-22`'s tuning pass). Still open: the composite-slider-to-raw-parameter blend ratios themselves (visual tuning, not a backend question, `D-22`).
...
3. **Phase 3 — Real DAGs + realtime + Apply to Batch**: Branching/parallel steps, SSE progress stream, presigned upload flow, and wiring the editor's recipe into the pipeline engine so "apply to batch" actually works — this is where the two halves of the project fuse. Presigned upload flow (`TASK-presigned-upload.md`) and Apply to Batch itself (`TASK-apply-to-batch.md`) landed 2026-08-07 — the editor's recipe now runs unmodified as a batch pipeline. The SSE progress stream (`TASK-realtime-progress-sse.md`) also landed 2026-08-07 — `GET /jobs/:id/events`, backed by its own `PLEXUS_JOB_EVENTS` NATS stream — but isn't wired into the batch view yet, which still polls `GET /jobs/:id` via `apps/web/src/lib/editor/batch-progress.ts`; swapping it to `apps/web/src/lib/jobs/useJobProgress.ts` is a follow-up. **Branching/parallel DAGs resolved 2026-08-07** (`docs/tasks/TASK-branching-parallel-dags.md`): `apps/orchestrator/src/pipelines/dag.validator.ts`'s `resolveDag()` replaces the old single-chain-only validator — real fan-out (multiple steps depending on the same parent) and multiple independent roots are both legal, `apps/orchestrator/src/jobs/job-dispatch.service.ts` dispatches every ready step in parallel per call, and a failed branch cascades `SKIPPED` to its descendants while independent sibling branches keep running, settling the job `PARTIAL` (a mix of success/failure) rather than failing the whole job outright. Fan-in (a step depending on more than one other step) stays unsupported — no built-in processor accepts more than one input yet, tracked as `docs/90-deferred-register.md` D-47. Phase 3 is now complete.
