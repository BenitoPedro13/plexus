...
| Objects | **MinIO** (self-hosted, resolved — see `docs/90-deferred-register.md` D-3) — presigned-URL upload/download via each side's own MinIO SDK (`minio-go/v7` in `workers/internal/storage`, the `minio` npm client in `apps/orchestrator/src/upload`), never proxy large files through the API. Deploy-target split (`TASK-deploy-railway.md`, D-16/D-41): local dev keeps this self-hosted MinIO (`infra/docker-compose.yml`); production uses a managed Railway Bucket instead — both are plain S3-compatible endpoints behind the same `MINIO_*` env vars, so this is a deploy-target choice, not a code or contract change. The one real difference, `MINIO_PATH_STYLE` (Railway's Bucket is virtual-host-style only; self-hosted MinIO is path-style), is env-driven on both SDKs |
